...
const LANG = "ja-JP"; // PowerPointの表(addTable)は、langが正しくないとCJK文字が表示されない(□になる)ことがあるため明示
...
function footer(slide, pageNum, opts = {}) {
  ...
  slide.addText(opts.credit || "© EXIDEA Inc.  |  CONFIDENTIAL", {
    ...
    fontFace: F, lang: LANG, fontSize: 8, color: dark ? "B9A7E0" : "9AA0AA", align: "left",
  });
  if (pageNum != null) {
    slide.addText(String(pageNum).padStart(2, "0"), {
      x: 12.35, y: 7.08, w: 0.45, h: 0.3, margin: 0,
      fontFace: F, lang: LANG, fontSize: 8, bold: true, color: dark ? "B9A7E0" : "9AA0AA", align: "right",
    });
  }
}
...
function header(slide, kicker, title, opts = {}) {
  ...
  slide.addText(kicker, {
    ...
    fontFace: F, lang: LANG, fontSize: 11, bold: true, color: kickColor, charSpacing: 2,
  });
  slide.addText(title, {
    x: 0.55, y: 0.62, w: 12.2, h: 0.78, margin: 0,
    fontFace: F, lang: LANG, fontSize: 25, bold: true, color: C.INK, lineSpacingMultiple: 1.08,
  });
}
...
function sectionDivider(slide, chapterNo, title, subtitle, pageNum) {
  ...
  slide.addText(chapterNo, {
    ...
    fontFace: F, lang: LANG, fontSize: 40, bold: true, color: "8E76C8",
  });
  slide.addText(title, {
    x: 0.9, y: 3.05, w: 11.4, h: 1.0, margin: 0,
    fontFace: F, lang: LANG, fontSize: 33, bold: true, color: C.WHITE, lineSpacingMultiple: 1.1,
  });
  if (subtitle) slide.addText(subtitle, {
    x: 0.92, y: 4.15, w: 10.8, h: 0.9, margin: 0,
    fontFace: F, lang: LANG, fontSize: 14, color: C.ICE, lineSpacingMultiple: 1.35,
  });
...
function numCircle(slide, x, y, n, color, size = 0.42) {
  ...
  slide.addText(String(n), {
    ...
    fontFace: F, lang: LANG, fontSize: size >= 0.4 ? 15 : 12, bold: true, color: C.WHITE, align: "center", valign: "middle",
  });
}
...
function statCallout(slide, x, y, w, num, label, source, color) {
  slide.addText(num, {
    ...
    fontFace: F, lang: LANG, fontSize: 44, bold: true, color: color || C.OUR, align: "left",
  });
  slide.addText(label, {
    x, y: y + 0.98, w, h: 0.65, margin: 0,
    fontFace: F, lang: LANG, fontSize: 12.5, bold: true, color: C.INK, lineSpacingMultiple: 1.2,
  });
  if (source) slide.addText(source, {
    x, y: y + 1.62, w, h: 0.5, margin: 0,
    fontFace: F, lang: LANG, fontSize: 8.5, color: C.MUTE, lineSpacingMultiple: 1.15,
  });
}
...
function pill(slide, x, y, w, text, bg, fg) {
  ...
  slide.addText(text, {
    ...
    fontFace: F, lang: LANG, fontSize: 10.5, bold: true, color: fg, align: "center", valign: "middle",
  });
}
...
function bandRow(slide, y, label, labelColor, desc, opts = {}) {
  ...
  slide.addText(label, { x: 0.75, y: y + 0.25, w: 2.85, h: 0.42, margin: 0, fontFace: F, lang: LANG, fontSize: 11, bold: true, color: C.WHITE, align: "center", valign: "middle" });
  slide.addText(desc, { x: 3.75, y: y + 0.13, w: 8.7, h: 0.68, margin: 0, fontFace: F, lang: LANG, fontSize: 10.5, color: C.INK, valign: "middle", lineSpacingMultiple: 1.25 });
}
...
function voiceCols(slide, y, rows) {
  ...
  rows.forEach((r, i) => {
    ...
    slide.addText(r.t, { x: x + 0.24, y: y + 0.16, w: 3.4, h: 0.62, margin: 0, fontFace: F, lang: LANG, fontSize: 12.5, bold: true, color: r.c, lineSpacingMultiple: 1.15 });
    slide.addText(r.b, { x: x + 0.24, y: y + 0.78, w: 3.4, h: 1.05, margin: 0, fontFace: F, lang: LANG, fontSize: 10, color: C.INK, lineSpacingMultiple: 1.28 });
  });
}
...
function llmoNeedMap(slide, pageNum) {
  ...
  slide.addText("ゴール：AIの回答に、正しい文脈で引用・推薦される（＝LLMOの成果）", { x: 0.8, y: 1.63, w: 11.7, h: 0.42, margin: 0, fontFace: F, lang: LANG, fontSize: 13, bold: true, color: C.WHITE, valign: "middle" });
  ...
  slide.addText("どれか1つでは効かない — 全レイヤーの“一貫性”が、LLMOの成否を分ける。", { x: 0.55, y: 6.55, w: 12.23, h: 0.4, margin: 0, fontFace: F, lang: LANG, fontSize: 12.5, bold: true, color: C.ACCENT, align: "center" });
...
function llmoCoverMap(slide, pageNum) {
  ...
  card(slide, 0.55, 1.58, 12.23, 0.52, { fill: C.NAVY });
  slide.addText("全レイヤーを1社で。だから“分析で終わらない”——実行して勝たせるところまで。", { x: 0.8, y: 1.63, w: 11.7, h: 0.42, margin: 0, fontFace: F, lang: LANG, fontSize: 13, bold: true, color: C.WHITE, valign: "middle" });
  voiceCols(slide, 2.24, [
  ...
  slide.addText("①基盤整理 → ②LLMO分析（無料→有料）→ ③自社発信 Emma Quick → ④クエリファンアウト対策 Emma SEO を一貫提供。", { x: 0.55, y: 6.55, w: 12.23, h: 0.4, margin: 0, fontFace: F, lang: LANG, fontSize: 12, bold: true, color: C.ACCENT, align: "center" });
  footer(slide, pageNum);
}

module.exports = { C, F, LANG, PAGE, BG_DOTS, BG_GRAD, newPres, footer, header, sectionDivider, vennMotif, numCircle, statCallout, card, pill, bandRow, voiceCols, llmoNeedMap, llmoCoverMap };
